      ...
      if (tbody && detailDiv) {
        tbody.querySelectorAll('tr').forEach(tr => {
          ...
          tr.addEventListener('click', async function () {
            const idx = parseInt(tr.getAttribute('data-row-idx'))
            const row = pageData[idx]
            // Always show the row data in the left area, even if passport_name is missing
            ...
            leftTable.innerHTML = `
              ...
              <form id="mkfMasterEditForm">
                <table style="border-collapse:collapse; background:#fff; border:1px solid #eee; font-size:1rem; width:100%;">
                  <tbody>
                    ${Object.entries(row)
                      .map(([key, value]) => `
                        <tr>
                          <th style="text-align:right; padding:4px 8px; background:#f5f5f5; color:${key === 'passport_number' ? '#d32f2f' : '#333'}; font-weight:${key === 'passport_number' ? 'bold' : 'normal'};\">${key}</th>
                          <td style="padding:4px 8px;">
                            <input type="text" name="${key}" value="${value ?? ''}" style="width:100%; background:#f5f5f5; border:1px solid #ccc; color:${key === 'passport_number' ? '#d32f2f' : '#333'}; font-weight:${key === 'passport_number' ? 'bold' : 'normal'};">
                          </td>
                        </tr>
                      `)
                      .join('')}
                ...
                <div style="text-align:right; margin-top:10px;">
                  <button type="submit" id="mkfMasterEditBtn" style="background:#4CAF50; color:#fff; border:none; padding:8px 20px; border-radius:4px; cursor:pointer;">수정</button>
                </div>
              </form>
            `

            // 수정 버튼 이벤트 핸들러
            const editForm = document.getElementById('mkfMasterEditForm')
            if (editForm) {
              editForm.onsubmit = async function(e) {
                ...
                // format_name이 없거나 빈 값이면 '오류 수정'으로 설정
                if (!data['format_name'] || data['format_name'].trim() === '') {
                  data['format_name'] = '오류 수정'
                }
                if (!data['passport_number'] || data['passport_number'].trim() === '') {
                  alert('passport_number가 없습니다.')
                  return
                }
                // passport_name이 없거나 빈 값이면 경고
                if (!data['passport_name'] || data['passport_name'].trim() === '') {
                  alert('passport_name이 없습니다.')
                  return
                }
                try {
                  const res = await fetch('/api/mkf-master-upsert', {
                    method: 'POST',
                  ...
                  if (res.ok && result.success) {
                    alert('수정이 완료되었습니다. (' + result.operation + ')')
                  } else {
