...
// mkf_master UPSERT (insert or update by passport_number)
app.post('/api/mkf-master-upsert', async (req, res) => {
  try {
    const data = req.body;
    if (!data.passport_number || data.passport_number.trim() === '') {
      return res.status(400).json({ error: 'passport_number는 필수입니다.' });
    }
    // 컬럼 목록 (DB 테이블 순서에 맞게 조정)
    const columns = [
      'id', 'format_name', 'nationality', 'passport_name', 'visa_type', 'passport_number',
      'phone_type', 'sim_price', 'deposit_amount', 'balance', 'loan_pre_priority',
      'entry_date', 'tel_number_kor', 'commit_date', 'mkf_status'
    ];
    // 값 추출 (빈 문자열 또는 undefined는 null로 변환, format_name은 '오류 수정' 기본값)
    const values = columns.map(col => {
      let v = data[col];
      if (col === 'format_name' && (!v || v === '')) {
        v = '오류 수정';
      }
    ...
    const setClause = columns
      .filter(col => col !== 'passport_number')
      .map((col, idx) => `${col} = $${idx + 1}`)
      .join(', ');
    const insertCols = columns.join(', ');
    const insertVals = columns.map((_, idx) => `$${idx + 1}`).join(', ');
    const updateCols = columns
      .filter(col => col !== 'passport_number')
      .map((col, idx) => `${col} = EXCLUDED.${col}`)
      .join(', ');
    const sql = `
      INSERT INTO mkf_master (${insertCols})
      VALUES (${insertVals})
    ...
    res.json({ success: true, operation: result.command === 'INSERT' ? 'insert' : 'update', row: result.rows[0] });
  } catch (err) {
    console.error('Error in /api/mkf-master-upsert:', err);
    res.status(500).json({ error: '서버 오류가 발생했습니다.' });
  }
});
